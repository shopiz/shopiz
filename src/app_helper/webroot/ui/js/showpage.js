	...
	switch (mode) {
		...
		case 16 : //模式4 (下拉框)
			if (this.pageCount < 1) {
				...
				var chkSelect;
				strHtml += '<select name="toPage" class="span1" onchange="' + this.name + '.toPage(this);">';
				for (var i = 1; i <= this.pageCount; i++) {
					if (this.page == i) chkSelect=' selected="selected"';
					else chkSelect='';
					strHtml += '    <option value="' + i + '"' + chkSelect + '>分页: ' + i + ' / ' + this.pageCount + '</option>';
				}
			}
			strHtml += '</select>';
			break;
		...
		case 64 : //模式6
			...
			if (this.page != this.pageCount) strHtml += '<li title="Page ' + this.pageCount + '"><a href="javascript:' + this.name + '.toPage(' + this.pageCount + ');">[' + this.pageCount + ']</a></li>';
			if (nextPage > this.pageCount) {
				...
				strHtml += '<li title="Last Page"><a href="javascript:' + this.name + '.toPage(' + this.pageCount + ');">»</a></li>';
			}
			strHtml += '</ul>';
			break;
		default :
